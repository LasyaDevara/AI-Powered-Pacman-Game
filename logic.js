...
var aiMoveInterval = 300; // milliseconds between AI moves

function preload() {
    blockImg = loadImage('assets/block.png');
    block2Img = loadImage('assets/block2.JPG');
    foodImg = loadImage('assets/food.png');
    powerImg = loadImage('assets/power.png');
    angrypacmanImg = loadImage('assets/pacman_tile.png');
    enemy1Img = loadImage('assets/ghost1.png');
    enemy2Img = loadImage('assets/ghost2.png');
    enemy3Img = loadImage('assets/ghost3.png');
    enemy1ImgWeak = loadImage('assets/ghost1fear.png');
    enemy2ImgWeak = loadImage('assets/ghost2fear.png');
    enemy3ImgWeak = loadImage('assets/ghost3fear.png');
}
...
function setup() {
    ...
    // Initialize AI
    pacmanAI = new PacmanAI(maze, standarSize);
    ...
    // Set up AI solver button
    document.getElementById('aiSolver').addEventListener('click', function() {
        isAIActive = !isAIActive;
        this.style.backgroundColor = isAIActive ? '#45a049' : '#6d4caf';
        this.textContent = isAIActive ? 'MANUAL CONTROL' : 'AI SOLVER';
        
        // Reset AI path when toggling
        aiPath = [];
        aiTarget = null;
    });
}
...
function draw() {
    ...
    // Apply grayscale to weak enemy images
    ...
    // AI Movement Logic
    if (isAIActive && millis() - lastAIMoveTime > aiMoveInterval) {
        let pacmanPos = {x: angrypacman.instance.x, y: angrypacman.instance.y};
        
        // Find new target if we don't have one or reached the current target
        if (!aiTarget || (aiTarget && dist(pacmanPos.x, pacmanPos.y, aiTarget.x, aiTarget.y) < standarSize/2)) {
            aiTarget = pacmanAI.findNearestTarget(pacmanPos, foods, powers);
            if (aiTarget) {
                aiPath = pacmanAI.findPath(pacmanPos, aiTarget, activeEnemies) || [];
            }
        }
        
        // Follow the path
        if (aiPath.length > 0) {
            let nextStep = aiPath[0];
            let dx = nextStep.x - pacmanPos.x;
            let dy = nextStep.y - pacmanPos.y;
            
            // Determine direction based on the next step
            if (dx > 0 && maze.maze[Math.floor(pacmanPos.y/standarSize)][Math.floor(pacmanPos.x/standarSize) + 1] !== '*') {
                angrypacman.instance.movePac(0); // Right
            } else if (dx < 0 && maze.maze[Math.floor(pacmanPos.y/standarSize)][Math.floor(pacmanPos.x/standarSize) - 1] !== '*') {
                angrypacman.instance.movePac(2); // Left
            } else if (dy > 0 && maze.maze[Math.floor(pacmanPos.y/standarSize) + 1][Math.floor(pacmanPos.x/standarSize)] !== '*') {
                angrypacman.instance.movePac(1); // Down
            } else if (dy < 0 && maze.maze[Math.floor(pacmanPos.y/standarSize) - 1][Math.floor(pacmanPos.x/standarSize)] !== '*') {
                angrypacman.instance.movePac(3); // Up
            }
            
            // Remove step if we're close to it
            if (dist(pacmanPos.x, pacmanPos.y, nextStep.x, nextStep.y) < standarSize/2) {
                aiPath.shift();
            }
        }
        
        lastAIMoveTime = millis();
    }
    ...
    // Debug: Draw AI path
    if (isAIActive && aiPath.length > 0) {
        stroke(255, 0, 0);
        strokeWeight(2);
        noFill();
        beginShape();
        vertex(angrypacman.instance.x + standarSize/2, angrypacman.instance.y + standarSize/2);
        for (let step of aiPath) {
            vertex(step.x + standarSize/2, step.y + standarSize/2);
        }
        endShape();
    }
}
...
function keyPressed() {
    if (isAIActive) return; // Ignore keyboard input when AI is active
    
    if(keyCode === RIGHT_ARROW) {
        // Check if next position is within bounds and not a wall
        if(angrypacman.instance.x/standarSize + 1 < maze.cols && 
           maze.maze[angrypacman.instance.y/standarSize][angrypacman.instance.x/standarSize + 1] !== '*') {
        ...
        // Check if next position is within bounds and not a wall
        if(angrypacman.instance.x/standarSize > 0 && 
           maze.maze[angrypacman.instance.y/standarSize][angrypacman.instance.x/standarSize - 1] !== '*') {
